# Next calibrated default candidate v22 — public post-repair preregistration
...
This is the public, redacted counterpart to the private normative post-repair preregistration. It
starts a fresh calibration cycle after the terminal wave-2 instrument failure. It is neither an
amendment nor a retry, and it authorizes no default change or candidate-universe access.
...
The wave-2 terminal record and partial artifacts remain immutable evidence of an instrument
failure, not a valid selection closure. They must not be used as an exclusion source or otherwise
consumed by this successor.

## Immutable cross-repository binding

The private normative authorization merged in Alfred at
`3f0f574752c406d336e47b51b09f7c724c13a843` (PR #910). Its two normative artifacts are:

| Private artifact | Git blob |
| --- | --- |
| `docs/calibration/next-default-v22-2026-08-12-post-repair/preregistration.md` | `6bdc462d86fa8c17ae7fe28dd847756f58ae32e4` |
| `docs/calibration/next-default-v22-2026-08-12-post-repair/selection-spec.json` | `6a1855b359088288e6a060e4e67bec70709f45ec` |
...
## Repair precondition and complete selection

Alfred commit `d9d0546d8a8505b849daf56409b200a656dfe451` repaired the freezer's optional-artifact
hash closure before this cycle was authorized. At that exact revision, a synthetic complete
closure produced a receipt only after independent verification; a missing artifact failed loudly
and produced no receipt. The private selection specification binds the repaired freezer and
independent verifier by blob.
...
private bindings. It uses fresh private seeds and excludes identities exposed by the retired
...
probe count is a lower bound on coverage, not a proof of complete no-egress. Before any execution
one-shot is spent, the frozen revision's driver test suite must pass, including the
missing-artifact loud-failure test; failure is terminal NO-GO and measures nothing about v22.

Any binding mismatch, metadata-integrity failure, source-boundary mismatch, failed control,
execution-integrity failure, missing output, identity-redaction failure, failed review or
estimator gate, replacement, retry after candidate execution begins, or post-result amendment is
terminal NO-GO. The terminal record must distinguish an instrument failure, which measures nothing
about v22, from a failed precommitted measurement gate.
