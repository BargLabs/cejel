...
RED on baseline with only the new test file: 12 tests, 0 passed, 12 failed (`MODULE_NOT_FOUND`, no guard existed). GREEN with implementation: 12 tests, 12 passed, 0 failed. Cases cover source edits, tests/fixtures control, marker edit/deletion/rename in both PR and full-tree modes, no-marker control, source rename, full-tree drift, unreadable revisions, shallow-history refusal, and a diverged-base PR control. PR changes use the merge base; newer base commits are not reversed into apparent PR deletions. Build passed; full Vitest suite passed 87 files / 1,341 tests, including the structural offline-boundary check. No scoring source changed.
