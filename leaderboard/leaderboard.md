...
- Every score on this board is produced through the same sealed public-scoring entry point used by `npx @cejel/cejel .`, with this board's explicit prospective rubric pin shown above. Ordinary public CLI calls that omit a rubric pin remain on calibrated v17. No private domain collector contributes. Public-repository rows pin an immutable public commit and are independently reproducible from it. The explicitly labeled private Alfred transparency snapshot is not independently reproducible because its source commit and evidence locations are not public; it is not presented as a public-repository self-score.
