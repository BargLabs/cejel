...
import { scanDeclaredButUnreadConfig } from '../dist/packs/d-series/index.js';
...
const checkoutRoot = resolve(args.get('--checkout-root') ?? '/tmp/cejel-d1-pinned-public');
const resultPath = resolve(args.get('--json') ?? '/tmp/cejel-d1-precision-gate.json');
...
if (publicEntries.length !== 23) {
  throw new Error(`d1_precision_denominator_mismatch:${publicEntries.length}`);
}
...
function prepareCheckout(entry) {
  ...
  if (actual !== entry.commit) {
    throw new Error(`d1_precision_pin_mismatch:${entry.name}:${actual}:${entry.commit}`);
  }
  return target;
}
...
for (const entry of publicEntries) {
  ...
  const findings = scanDeclaredButUnreadConfig(checkout);
  repositories.push({
    name: entry.name,
    url: entry.url,
...
const result = {
  schemaVersion: 'cejel-d1-precision-gate-v1',
  ruleId: 'D1',
