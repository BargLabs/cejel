...
import { scanSelfReferentialVerification } from '../dist/packs/d-series/index.js';
...
const checkoutRoot = resolve(args.get('--checkout-root') ?? '/tmp/cejel-d5-pinned-public');
const resultPath = resolve(args.get('--json') ?? '/tmp/cejel-d5-precision-gate.json');
...
if (publicEntries.length !== 23) {
  throw new Error(`d5_precision_denominator_mismatch:${publicEntries.length}`);
}
...
function prepareCheckout(entry) {
  ...
  if (actual !== entry.commit) {
    throw new Error(`d5_precision_pin_mismatch:${entry.name}:${actual}:${entry.commit}`);
  }
  return target;
}
...
for (const entry of publicEntries) {
  ...
  const findings = scanSelfReferentialVerification(checkout);
  repositories.push({
    name: entry.name,
    url: entry.url,
...
const findingCount = repositories.reduce((total, repository) => total + repository.findings.length, 0);
const result = {
  schemaVersion: 'cejel-d5-precision-gate-v1',
  ruleId: 'D5',
...
process.stdout.write(`result=${result.result} findings=${findingCount}/23 output=${basename(resultPath)}\n`);
